...
SELECT id, name FROM categories WHERE EXISTS (SELECT * FROM offers_category WHERE offers_category.category_id = categories.id);
...
SELECT
  ...
  u.name,
  u.email,
  (SELECT COUNT(*) FROM comments WHERE offer_id = o.id) as comment_count,
  string_agg(DISTINCT ct.name, ',') as categories
  FROM offers o
  LEFT JOIN users u ON o.author_id = u.id
  LEFT JOIN offers_category oct ON o.id = oct.offer_id
...
SELECT
  ...
  o.description,
	...
	LEFT JOIN categories ct ON oct.category_id = ct.id
  WHERE o.id = 5
	GROUP BY o.id, u.name, u.email, u.email;
